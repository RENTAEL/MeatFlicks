...
const memberDiag = () => b.evaluate(() => (window.__wpDiag ? window.__wpDiag.map((d) => ({ ...d })).slice(-12) : [])).catch(() => []);
const esInfo = () => b.evaluate(() => ({
	handlerSeen: (window.__appHandlerSeen || []).slice(-12),
	events: (window.__esEvents || []).slice(-8)
})).catch(() => null);
...
const poll = async (label) => {
	...
	console.log(`t+${Math.round((Date.now() - t0) / 1000)}s roomSeq=${rs?.playback?.seq} | memberHook=${JSON.stringify(mh)}`);
	console.log(`  appHandlerSeen=${JSON.stringify(es?.handlerSeen)}`);
	console.log(`  es=${JSON.stringify(es?.events)}`);
	console.log(`  memberDiag(last6)=${JSON.stringify(md.slice(-6))}`);
};
