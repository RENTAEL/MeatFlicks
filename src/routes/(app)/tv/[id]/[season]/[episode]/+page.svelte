<script lang="ts">
	...
	let currentEpIndex = $derived(episodes.findIndex(
		(e: any) => e.season_number === seasonNum && e.episode_number === episodeNum
	));
	...
	<div class="ep-page">
		...
		<div class="ep-nav-header">
			...
			<div class="ep-nav-buttons">
				<button class="ep-nav-btn" disabled={!hasPrev} onclick={() => goEpisode(episodes[currentEpIndex - 1])}>
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
					Prev
				</button>
				<button class="ep-nav-btn" disabled={!hasNext} onclick={() => goEpisode(episodes[currentEpIndex + 1])}>
					Next
		...
		<div class="ep-section">
			...
			<div class="ep-list" class:ep-list-scroll={episodes.length > 15}>
				{#each episodes as ep (ep.id)}
					<button
						class="ep-list-item"
						class:ep-list-item-active={ep.season_number === seasonNum && ep.episode_number === episodeNum}
						onclick={() => goEpisode(ep)}
					>
						<div class="ep-list-still">
							{#if ep.still_path}
								<img src="https://image.tmdb.org/t/p/w185{ep.still_path}" alt="" class="ep-list-still-img" />
							{/if}
						</div>
...
<style>
	...
	.ep-nav-header { display: flex; align-items: center; justify-content: space-between; padding: 0 16px 12px; gap: 12px; }
	.ep-title { font-size: 15px; font-weight: 600; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.ep-nav-buttons { display: flex; gap: 8px; flex-shrink: 0; }
	.ep-nav-btn { display: flex; align-items: center; gap: 4px; padding: 8px 14px; background: #18181b; border: 1px solid #27272a; border-radius: 8px; color: #a1a1aa; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; }
	.ep-nav-btn:disabled { opacity: 0.3; cursor: not-allowed; }
	.ep-nav-btn:not(:disabled):active { background: #27272a; }
	.ep-nav-btn svg { width: 14px; height: 14px; }
	...
	.ep-list-still-img { width: 100%; height: 100%; object-fit: cover; }
	.ep-list-info { flex: 1; min-width: 0; display: flex; align-items: center; gap: 8px; }
	...
	.ep-info-skel { padding: 16px; }
	.skel-line { height: 14px; border-radius: 6px; background: #18181b; animation: pul 1.5s infinite; }
