<script lang="ts">
  ...
  const browser = typeof navigator !== 'undefined'
    ? (navigator as { userAgentData?: { brands?: { brand: string }[] } }).userAgentData?.brands?.[0]?.brand ?? navigator.userAgent
    : '';

  const isSupported = /Chrome|Edge|Brave|Chromium/i.test(browser) && !/iOS|iPhone|iPad/i.test(browser);
...
{#if visible}
  <div class="popup-backdrop" onclick={dismiss} onkeydown={(e) => e.key === 'Escape' && dismiss()} role="button" tabindex="-1" aria-label="Close" transition:fade={{ duration: 150 }}></div>

  <div class="popup" transition:fly={{ y: 30, duration: 250 }}>
    ...
    <p class="popup-text">
      ...
      <strong>uBlock Origin Lite</strong> to block them automatically.
      It's free, lightweight, and takes 10 seconds.
    </p>
...
<style>
  ...
  .popup-close:hover { background: rgba(129, 140, 248, 0.1); color: #e0e7ff; }
  .popup-close svg { width: 18px; height: 18px; }

  .popup-icon { font-size: 40px; margin-bottom: 12px; }
  .popup-title { font-size: 20px; font-weight: 700; color: #e0e7ff; margin: 0 0 8px; }
  .popup-text { font-size: 14px; color: #a5b4fc; line-height: 1.6; margin: 0 0 20px; }
  .popup-text strong { color: #e0e7ff; }
