...
export function drawQuoteBanner(
	...
	// CTA — the ask: visit the site for more daily quotes
	const ctaY = height - Math.round(height * 0.11);
	...
	ctx.textBaseline = 'middle';
	ctx.fillText(QUOTE_BANNER_CTA, width / 2, ctaY + 2);
	ctx.textBaseline = 'alphabetic';

	// site url
	ctx.fillStyle = '#818cf8';
	ctx.font = `500 ${Math.round(width * 0.024)}px ui-monospace, monospace`;
	ctx.fillText(QUOTE_BANNER_SITE_URL, width / 2, height * 0.945);
}
