...
export const themes: Record<ThemeId, ThemeColors> = {
	...
	light: {
		...
		accent: '#6c4ce0',
		accentHover: '#7d5ff0',
		accentGlow: 'rgba(108, 76, 224, 0.15)',
	...
	neon: {
		...
		accent: '#00ffc8',
		accentHover: '#33ffd6',
		accentGlow: 'rgba(0, 255, 200, 0.3)',
		gradientBrand: 'linear-gradient(135deg, #00ffc8, #b44dff)',
		gradientBrandHorizontal: 'linear-gradient(90deg, #00ffc8, #b44dff)',
		border: 'rgba(180, 77, 255, 0.1)',
		borderStrong: 'rgba(180, 77, 255, 0.2)',
		shadowSm: '0 2px 8px rgba(0, 0, 0, 0.4)',
		shadowMd: '0 4px 16px rgba(180, 77, 255, 0.1)',
		shadowLg: '0 8px 32px rgba(0, 255, 200, 0.05)',
		success: '#00ffc8',
		danger: '#ff4477',
		warning: '#ffbb33'
	},
	forest: {
		...
		borderStrong: 'rgba(74, 222, 128, 0.16)',
	...
	sunset: {
		...
		shadowSm: '0 2px 8px rgba(0, 0, 0, 0.35)',
		shadowMd: '0 4px 16px rgba(245, 158, 11, 0.08)',
		shadowLg: '0 8px 32px rgba(0, 0, 0, 0.5)',
		success: '#4ade80',
		danger: '#f87171',
		warning: '#f59e0b'
	...
	ocean: {
		...
		shadowSm: '0 2px 8px rgba(0, 0, 0, 0.35)',
		shadowMd: '0 4px 16px rgba(56, 189, 248, 0.08)',
		shadowLg: '0 8px 32px rgba(0, 0, 0, 0.5)',
		success: '#4ade80',
		danger: '#f87171',
		warning: '#fbbf24'
	...
	lavender: {
		...
		borderStrong: 'rgba(192, 132, 252, 0.16)',
		shadowSm: '0 2px 8px rgba(0, 0, 0, 0.35)',
		shadowMd: '0 4px 16px rgba(192, 132, 252, 0.08)',
		shadowLg: '0 8px 32px rgba(0, 0, 0, 0.5)',
		success: '#4ade80',
		danger: '#f87171',
		warning: '#fbbf24'
	...
	sofia: {
		// Sofia the First series look — Enchancia royal plum, amulet violet, rose
		// and crown gold. This is the default for the cocolemon/Sofia brand.
		name: 'sofia',
		label: 'Sofia',
		emoji: '👑',
		isDark: true,
		...
		bgInput: '#1a0f2c',
		textPrimary: '#f6edff',
		textSecondary: '#cdb9ea',
		textTertiary: '#8f78b4',
		accent: '#c8a8ff',
		accentHover: '#d9c0ff',
		accentGlow: 'rgba(200, 168, 255, 0.25)',
		gradientBrand: 'linear-gradient(135deg, #fcd34d, #f472b6, #a78bfa)',
		gradientBrandHorizontal: 'linear-gradient(90deg, #fcd34d, #f472b6, #a78bfa)',
		border: 'rgba(200, 168, 255, 0.1)',
		borderStrong: 'rgba(200, 168, 255, 0.2)',
		shadowSm: '0 2px 8px rgba(0, 0, 0, 0.35)',
		shadowMd: '0 4px 16px rgba(0, 0, 0, 0.45)',
...
function darken(hex: string, factor: number): string {
	const v = parseInt(hex.replace('#', ''), 16);
	const c = (x: number) =>
		Math.round(Math.min(255, x * factor))
			.toString(16)
			.padStart(2, '0');
	return `#${c((v >> 16) & 255)}${c((v >> 8) & 255)}${c(v & 255)}`;
}

/** Scale a hex color's channels by `factor` (lighten > 1, darken < 1). */
...
function primaryPair(accent: string): { primary: string; foreground: string } {
	...
	for (let factor = 0.92; factor > 0.3; factor -= 0.04) {
		...
		if (ratio(luminance(primary), luminance(WHITE)) >= AA_WHITE)
			return { primary, foreground: WHITE };
	}
